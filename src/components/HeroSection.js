            ...
            <section className="hero bg-color" data-id={_.get(section, 'section_id', null)}>
                ...
                <div className="hero__links link-group">
                  {_.map(_.get(section, 'actions', null), (action, action_idx) => {
                      ...
                      return (
                        <Link style={{backgroundColor: "red"}} key={action_idx} href={withPrefix(_.get(action, 'url', null))} className={classNames('link', {'link--filled': action_style === 'primary', 'link--borderless': action_style === 'link'})}>
